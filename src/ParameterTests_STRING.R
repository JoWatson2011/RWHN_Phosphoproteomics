...
stringTests_hetNet <- lapply(seq(0.1, 0.9, 0.1), function(x){
  ...
  hetNet <- constructHetNet(clustering = cl$cluster, stringConf = x, modules = F)
})
...
#Save
saveRDS(stringTests_RWHN, "results/data/STRINGtests.rds")
...
stringTests_RWHN <- readRDS("results/data/STRINGtests.rds")
dat <- expand.grid(
  data.frame(res1 = 1:length(stringTests_RWHN),
             res2 = 1:length(stringTests_RWHN)))

wt <- lapply(1:max(cl$cluster), function(clus){
...
egf_nw <- graph_from_data_frame(egf_mlnw$edgelists$y, directed = F)#, egf_mlnw$v[egf_mlnw$v$layer == "prot",])
components(egf_nw)$csize
tgf_nw <- graph_from_data_frame(tgf_mlnw$edgelists$y, directed = F)#, egf_mlnw$v[egf_mlnw$v$layer == "prot",])
components(tgf_nw)$csize